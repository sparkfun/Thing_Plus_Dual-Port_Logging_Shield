...
// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  // Set the default mode to USB "memory stick" (SDIO) mode
  //
  // Returns 0 if the write was successful

  Serial.println(F("Setting the default mode to SDIO (USB \"memory stick\" mode)"));
...
// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  Serial.print(F("Waiting 15 seconds"));
  for (int i = 0; i < 15; i++)
  {
    delay(1000);
    Serial.print(F("."));
